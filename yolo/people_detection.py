...
def write(x, img, classes, only_person=False):
    c1 = tuple(x[1:3].int())
    c2 = tuple(x[3:5].int())
    cls = int(x[-1])
    if (not only_person) or (only_person and classes[cls] == 1):
...
def run(frame):
    ...
    weightsfile = "yolo/yolov3.weights"
    num_classes = 80
...
#            im_dim = im_dim.repeat(output.size(0), 1)
    ...
    list(map(lambda x: write(x, orig_im, classes, only_person=True), output))
